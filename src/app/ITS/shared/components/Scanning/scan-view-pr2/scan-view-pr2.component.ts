...
@Component({
  selector: 'app-scan-view-pr2',
  templateUrl: './scan-view-pr2.component.html',
  styleUrls: ['./scan-view-pr2.component.scss']
})
export class ScanViewPr2Component implements OnInit, OnDestroy {
  ...
  private onScanComplete(): void {
    ...
    if (this.scannedTags.length > 0) {
      this._scanviewService.getFilteredItemInfoByEPC(this.scannedTags, this.productLotNo)
      .pipe(takeUntil(this.destroyed$))
      .subscribe({
        next: (items) => {
