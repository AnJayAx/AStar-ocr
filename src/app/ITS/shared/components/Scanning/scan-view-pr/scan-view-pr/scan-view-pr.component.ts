...
import { ScanViewService } from '../../scan-view/scan-view.service';
//import { ScanViewInvalidTagsService } from './scan-view-invalid-tags.service';
import { ScanViewInvalidTagsService } from '../../scan-view/scan-view-invalid-tags.service';
import { CustomDialogService } from '@dis/services/message/custom-dialog.service';

@Component({
  selector: 'app-scan-view-pr',
  templateUrl: './scan-view-pr.component.html',
  styleUrls: ['./scan-view-pr.component.scss']
})
export class ScanViewPrComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.productLotNo = this._scanviewService.getProductLotNo();
  }
  ...
  private onScanComplete(): void {
    ...
    if (!this.productLotNo) {
      alert("Production LotNo is null. Aborting scan.");
      return;
  }

    if (this.scannedTags.length > 0) {
      this._scanviewService.getFilteredItemInfoByEPCLot(this.scannedTags, this.productLotNo)
      .pipe(takeUntil(this.destroyed$))
      .subscribe({
        next: (items) => {
