...
@Component({
  selector: 'app-productreceive-scan',
  templateUrl: './productreceive-scan.component.html',
  styleUrls: ['./productreceive-scan.component.scss']
})
export class ProductreceiveScanComponent implements OnInit, OnDestroy {
  private destroyed$: Subject<boolean> = new Subject();
  
  private scannedItems: IItemInfo[] = [];
  private formItems: ISelectedTag[]= [];

//   public CIO_FILTERBYAND: IScanViewListFilter[] = [
//     {property: 'IsIndividual', value: 'M'},
//   ];
  ...
  constructor(
      ...
      this._layoutService.changeTitleDisplayAndSetNavBackPath('Product Receive', 'mainmenunew');
  }
  ...
  onConfirm(confirmed: boolean) { 
      if (confirmed && this.scannedItems.length > 0) {
          ...
          this.router.navigate(['/productreceiveform']);
      }
  }
