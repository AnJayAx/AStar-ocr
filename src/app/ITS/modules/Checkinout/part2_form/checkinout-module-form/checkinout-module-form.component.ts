...
@Component({
  selector: 'app-checkinout-module-form',
  templateUrl: './checkinout-module-form.component.html',
  styleUrls: ['./checkinout-module-form.component.scss'],
  providers: [CheckinoutService]
})
export class CheckinoutModuleFormComponent implements OnInit, OnDestroy {
  ...
  checkIn: boolean;  // check in out boolean vars
  ...
  constructor(
    ...
    this._layoutService.changeTitleDisplayAndSetNavBackPath('Check In/Out Details', 'checkinout');
  }
  ...
  pressSubmit() {
    ...
    else if (this._checkinoutService.containsSAvailableItem(this.retrievedTags) && this.checkIn) {
      this._customDialog.message(
    ...
    else if (this._checkinoutService.containsCheckOut(this.retrievedTags) && this.checkIn === false) {
      this._customDialog.message(
  ...
  onClose(closeType: DialogCloseEventType) {
    if (closeType === DialogCloseEventType.Submit) {
      ...
      if (this.checkIn === true) {
        this._checkinoutService.postCheckIn(result).pipe(takeUntil(this.destroyed$)).subscribe({
          next: (response) => { 
            console.log('Check in posted');
            if (response.primary) {
              this._checkinoutService.clearTags();
        ...
        this._checkinoutService.postCheckOut(result).pipe(takeUntil(this.destroyed$)).subscribe({
          next: (response) => { 
